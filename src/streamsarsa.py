...
@jit
def get_delta(q_network, scaled_reward, gamma, done, obs, action, next_obs, next_action):
    q_sp = q_network(next_obs)[next_action]
...
class StreamSarsaTrainState(eqx.Module):
    key: chex.PRNGKey
    done: bool
    obs: chex.PRNGKey
    ...
    def replace(self, **kwargs) -> 'StreamSarsaTrainState':
...
class StreamSarsa:
    ...
    @classmethod
    def create(cls, **kwargs) -> "StreamSarsa":
    ...
    def make_act(self, train_state: StreamSarsaTrainState) -> Callable[[chex.Array, chex.PRNGKey], int | float | chex.Array]:
    ...
    def make_normalizer(self, train_state: StreamSarsaTrainState) -> Callable[[chex.Array], chex.Array]:
    ...
    def train(self, key: chex.PRNGKey) -> StreamSarsaTrainState:
        @eqx.filter_jit
        def train_iteration(ts: StreamSarsaTrainState):
            ...
            next_obs, next_state, reward, done, _ = self.env.step(step_key, state, ts.action, self.env_params)
            ...
            next_action, q_value, explored = q_epsilon_greedy(q_network, obs, eps, action_key)

            # Update eligibility trace
            td_error, td_grad = value_and_grad(get_delta)(q_network, scaled_reward, self.gamma, done, obs, ts.action, next_obs, next_action)
        ...
        @eqx.filter_jit
        def eval_iteration(ts: StreamSarsaTrainState):
        ...
        # get first action
        key, action_key = jax_random.split(key)
        action, _, _ = q_epsilon_greedy(q_network, obs, 1.0, action_key)

        reward_stats = SampleMeanStats.new_params(())
        train_state = StreamSarsaTrainState(
            key=key_ts,
            done=False,
            obs=obs,
            state=state,
